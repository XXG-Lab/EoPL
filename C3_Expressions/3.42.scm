...
(define init-clean-senv
  (lambda (env)
    (if (null? env)
        '()
        (cons #f (init-clean-senv (cdr env))))))

(define mark-senv
  (lambda (clean-senv n)
    (if (zero? n)
        (cons #t (cdr clean-senv))
        (cons (car clean-senv) (mark-senv (cdr clean-senv) (- n 1))))))

(define (reduce-left func init lst)
  (if (null? lst)
      init
      (reduce-left func (func init (car lst)) (cdr lst))))

(define clean-senv
  (lambda (vars body env)
    (define (add-vars body clean-env)
      (cases expression body
        (const-exp (num) clean-env)
        (var-exp (var) (mark-senv clean-env (car (apply-senv env var))))
        (add-exp (exp1 exp2) (add-vars exp1 (add-vars exp2 clean-env)))
        (diff-exp (exp1 exp2) (add-vars exp1 (add-vars exp2 clean-env)))
        (zero?-exp (exp1) (add-vars exp1 clean-env))
        (equal?-exp (exp1 exp2) (add-vars exp1 (add-vars exp2 clean-env)))
        (less?-exp (exp1 exp2) (add-vars exp1 (add-vars exp2 clean-env)))
        (greater?-exp (exp1 exp2) (add-vars exp1 (add-vars exp2 clean-env)))
        (if-exp (exp1 exp2 exp3) (add-vars exp1 (add-vars exp2 (add-vars exp3 clean-env))))
        (cond-exp (conds exps)
                  (let* ((reduce-func (lambda (env exp) (add-vars exp env)))
                         (env1 (reduce-left reduce-func clean-env conds))
                         (env2 (reduce-left reduce-func env1 exps)))
                    env2))
        (cons-exp (exp1 exp2) (add-vars exp1 (add-vars exp2 clean-env)))
        (emptylist-exp () (clean-env))
        (unpack-exp (vars exp1 exp2) (add-vars exp1 (add-vars exp2 clean-env)))
        (let-exp (vars exps body)
                  (let* ((reduce-func (lambda (env exp) (add-vars exp env)))
                         (env1 (reduce-left reduce-func clean-env exps))
                         (env2 (add-vars body env1)))
                    env2))
        (proc-exp (vars body) (add-vars body clean-env))
        ...
        (call-exp (rator rands)
                  (let* ((reduce-func (lambda (env exp) (add-vars exp env)))
                         (env1 (reduce-left reduce-func clean-env rands))
                         (env2 (add-vars rator env1)))
                    env2))
        (else (error 'translation-of "Invalid expression: " body))))
    (let ((clean-env (add-vars body (init-clean-senv env))))
      (if (null? vars)
          clean-env
          (mark-senv clean-env (car (apply-senv env (car vars))))))))
...
(define reduce-nameless-env
  (lambda (env clean-env)
    (if (or (null? clean-env)
            (null? env))
        '()
        (if (car clean-env)
            (cons (car env) (reduce-nameless-env (cdr env) (cdr clean-env)))
            (cons '() (reduce-nameless-env (cdr env) (cdr clean-env)))))))
    ...
    (cases expression exp
      ...
      (letrec-exp (names varss exps body)
                  (let ((named-env (extend-senv-letrec names senv)))
                    ...
                    (define (clean-rec varss exps)
                      (if (null? exps)
                          '()
                          (cons (clean-senv (car varss)
                                            (car exps)
                                            (extend-senv (car varss) named-env))
                                (clean-rec (cdr varss) (cdr exps)))))
                    (nameless-letrec-exp (exp-rec varss exps)
                                         (translation-of body named-env)
                                         (clean-rec varss exps))))
      (call-exp (rator rands)
                (call-exp
...
(define value-of
  (lambda (exp nameless-env)
    (cases expression exp
      ...
      (nameless-proc-exp (body clean)
                         (proc-val (procedure body (reduce-nameless-env nameless-env clean))))
      (nameless-letrec-exp (exps body cleans)
                           (let ((procs (map-of-two (lambda (exp clean)
                                               (proc-val
                                                (procedure exp
                                                           (reduce-nameless-env nameless-env clean))))
                                             exps cleans)))
