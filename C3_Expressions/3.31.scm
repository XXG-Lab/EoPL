...
(define (apply-env env search-var)
  (cases environment env
    ...
    (extend-env-rec (name vars body old-env)
  ...
  (cases environment env
    ...
    (extend-env-rec (name vars body old-env)
                    (if (eqv? name search-var)
                        (remove-from-env old-env)
                        (extend-env-rec name vars body (remove-from-env old-env search-var))))))
...
(define (combine-env env1 env2)
  (cases environment env1
    ...
    (extend-env-rec (name vars body old-env)
                    (extend-env-rec name vars body (combine-env old-env env2)))))
...
(define grammar
  '((expression (number) const-exp)
    ...
    (expression ("letproc" identifier "(" (arbno identifier) ")" expression "in" expression)
                letproc-exp)
    (expression ("letrec" identifier "(" (arbno identifier) ")" "=" expression "in" expression)
                letrec-exp)
  ...
  (lambda (exp env)
    (cases expression exp
      ...
      (traceproc-exp (vars body)
                     ...
                     (print body)
                     (display "\n")
                     (let ((result (proc-val (procedure vars body (clean-env vars body env)))))
                       (print "Leave: ")
                       (print vars)
                       (print body)
                       (display "\n")
      ...
      (letrec-exp (name vars exp body)
                  (value-of body
                            (extend-env-rec name vars exp env)))
      (call-exp (rator rand)
...
(equal? (expval->val (run "letrec double(x)
                                   = if zero?(x) then 0 else -((double -(x,1)), minus(2))
                           in (double 6)")) 12)

(equal? (expval->val (run "letrec gcd(x y)
                                   = if equal?(x, y)
                                     then x
                                     else if greater?(x, y)
                                          then (gcd -(x, y) y)
