...
(define grammar
  '((expression (number) const-exp)
    ...
    (bool-expression ("null?" "(" expression ")") null?-exp)
    ...
    (cases expression exp
      ...
      (cond-exp (conds exps)
                (cond ((null? conds) (error 'cond "No condition matched"))
                      ((expval->bool (value-of-bool-exp (car conds) env))
...
(equal? (expval->val (run "if greater? (1, 2) then 1 else 2")) 2)
