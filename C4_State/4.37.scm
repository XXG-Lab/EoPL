...
(define (apply-result rands refs env store)
  (if (null? rands)
      store
      (cases expression (car rands)
        (nameless-var-exp (n pos)
                          (let* ((ref (apply-env env n pos))
                                 (val (deref store (car refs)))
                                 (next-store (setref store ref val)))
                          (apply-result (cdr rands) (cdr refs) env next-store)))
        (else (apply-result (cdr rands) (cdr refs) env store)))))

(define (apply-procedure-ref proc1 rands vals env store)
  (cases proc proc1
    (procedure (body saved-env)
               (let* ((ret (extend-env-ref vals saved-env store))
                      (rands-env (car ret))
                      (rands-store (cadr ret))
                      (body-ret (value-of body rands-env rands-store))
                      (val (get-val body-ret))
                      (body-store (get-store body-ret))
                      (refs (car rands-env))
                      (next-store (apply-result rands refs env body-store)))
                 (list val next-store)))
    (subroutine (body saved-env)
               (let* ((ret (extend-env-ref vals saved-env store))
                      ...
                      (val (get-val body-ret))
                      (body-store (get-store body-ret))
                      (env-vals (caddr body-ret))
                      (next-store (apply-result rands env-vals env body-store)))
                 (list val next-store)))))
    ...
    (cases expression exp
      ...
      (callref-exp (rator rands)
                   (let* ((ret-rator (value-of rator env store))
                          ...
                          (vals (get-val ret-rand))
                          (store-end (get-store ret-rand)))
                     (apply-procedure-ref proc rands vals env store-end)))
      (nameless-var-exp (n pos)
...
in  let g = proc(y) callref(f y)
    in  let z = 55
        in begin callref(g z); z end
}")
(test program-1 44)
