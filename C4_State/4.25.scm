...
(define grammar
  '((program (statement) a-program)
    ...
    (statement ("var" (separated-list identifier "=" expression ",") ";" statement) var-stat)))
    ...
    (cases statement stat
      ...
      (dowhile-stat (stat exp)
                    (dowhile-stat (translation-of-stat stat senv)
                                  (translation-of-exp exp senv)))
      (var-stat (vars exps stat)
                (let ((next-senv (extend-senv vars senv)))
                  (nameless-var-stat (map (lambda (exp) (translation-of-exp exp senv)) exps)
                    ...
                    (define (while-exp-rec store)
                      (let* ((stat-ret (result-of stat env store))
                        ...
                        (if (expval->bool val)
                            ...
                            (list val next-store))))
                    (while-exp-rec store))
      (nameless-var-stat (exps stat)
                         (let* ((rets (value-of-list exps env store))
                                (vals (get-val rets))
                                (exps-store (get-store rets))
                                (ret (extend-env-ref vals env exps-store))
...
(define program-1 "var x = 3, y = 4; {
                       print +(x,y)
                   }")
(test program-1 7)

(define program-2 "var x = 3, y = 4, z = 0; {
                       while not(zero?(x)) {
                           z = +(z, y);
                           x = -(x, 1)
                       };
...
(define program-4 "var f = proc(x y) *(x, y), x = 3; {
                       print (f 4 x)
                   }")
(test program-4 12)
