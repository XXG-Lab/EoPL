...
(define grammar
  '((program ((arbno module-definition) expression) a-program)
    ...
    (module-body ("depends-on" (separated-list identifier ",") module-body) depend-module-body)
...
(define-datatype module-body module-body?
  ...
  (sub-module-body (defn module-definition?)
                   (body module-body?))
  (depend-module-body (vars (list-of identifier?))
                      (body module-body?)))
...
(define (filter-env-with-module vars env)
  (cases environment env
    (empty-env ()
               env)
    (extend-env-ref (var ref saved-env)
                    (extend-env-ref var ref (filter-env-with-module vars saved-env)))
    (extend-env-with-module (name val saved-env)
                            (if (exist? name vars)
                                (extend-env-with-module name val
                                                        (filter-env-with-module vars saved-env))
                                (filter-env-with-module vars saved-env)))))
...
(define (interface-of body tenv)
  (cases module-body body
    ...
    (depend-module-body (vars body)
                        (interface-of body (filter-tenv-with-module vars tenv)))))
...
(define (<:-decl decl decls tenv)
  (if (null? decls)
      #f
      (let ((name1 (declaration->name decl))
            (name2 (declaration->name (car decls)))
            (type1 (declaration->type decl))
            (type2 (declaration->type (car decls))))
        (if (and (eqv? name1 name2)
                 (equal? type1 type2))
            #t
            (<:-decl decl (cdr decls) tenv)))))

(define (<:-decls decls1 decls2 tenv)
  (if (null? decls2)
      #t
      (and (<:-decl (car decls2) decls1 tenv)
           (<:-decls decls1 (cdr decls2) tenv))))

(define (filter-tenv-with-module vars tenv)
  (cases type-env tenv
    (empty-tenv ()
                tenv)
    (extend-tenv (var ty saved-tenv)
                 (extend-tenv var ty (filter-tenv-with-module vars saved-tenv)))
    (extend-tenv-with-module (name interface saved-tenv)
                             (if (exist? name vars)
                                 (extend-tenv-with-module name interface
                                                          (filter-tenv-with-module vars saved-tenv))
                                 (filter-tenv-with-module vars saved-tenv)))))
...
(define (value-of-module-body iface body env)
  (cases module-body body
    ...
    (depend-module-body (vars body)
                        (value-of-module-body iface body (filter-env-with-module vars env)))))
...
module m2
  ...
  body
    ...
    [v = from m1 take u]
1")
(equal?! prog-depends (cons 32 'int))
