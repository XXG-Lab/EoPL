...
(define-datatype interface interface?
  ...
  (proc-interface (param-name identifier?)
                       ...
                       (cases interface rator-iface
                         ...
                         (proc-interface (param-name param-iface result-iface)
                                     (if (<:-iface rand-iface param-iface tenv)
                                         ...
                                         (error param-iface rand-iface m-body))))))
...
(define (rename-in-iface m-type old new)
  (cases interface m-type
    (an-interface (decls)
                  (an-interface
                   (rename-in-decls decls old new)))
    (proc-interface (param-name param-type result-type)
                    (proc-interface 
                     param-name
      ...
      (let ((decl (car decls))
        ...
        (cases declaration decl
          (val-declaration (name ty)
                           (cons
                            (val-declaration name (rename-in-type ty old new))
                            (rename-in-decls decls old new)))
          (opaque-declaration (name)
                              (cons
                               ...
                               (if (eqv? name old)
                                   decls
                                   (rename-in-decls decls old new))))
          (transparent-declaration (name ty)
                                   (cons
...
(define-datatype module-body module-body?
  ...
  (proc-module-body (name identifier?)
                    (type interface?)
                    (body module-body?))
  (var-module-body (name identifier?))
  (app-module-body (rator identifier?)
                   (rand identifier?)))
...
(define (find-decl decl decls)
  (let ((name1 (declaration->name decl)))
    (define (find-decl-rec decls)
      (if (null? decls)
          #f
          (let ((name2 (declaration->name (car decls))))
            (if (eqv? name1 name2)
                (car decls)
                (find-decl-rec (cdr decls))))))
    (find-decl-rec decls)))

(define (remove-decl decl decls)
  (let ((name1 (declaration->name decl)))
    (define (remove-decl-rec decls)
      (if (null? decls)
          '()
          (let ((name2 (declaration->name (car decls))))
            (if (eqv? name1 name2)
                (remove-decl-rec (cdr decls))
                (cons (car decls)
                      (remove-decl-rec (cdr decls)))))))
    (remove-decl-rec decls)))

(define (<:-decls decls1 decls2 tenv)
  (cond ((null? decls2) #t)
        ((null? decls1) #f)
        (else
         (let ((decl (find-decl (car decls1) decls2)))
           (if decl
               (and (<:-decl (car decls1) decl tenv)
                    (<:-decls (cdr decls1) (remove-decl decl decls2)
                         ...
                         (proc-module (m-name m-body env)
                                      (value-of-module-body m-body
                                                            (extend-env-with-module
                                                             m-name rand-val env)))
                         (else
                          (error 'value-of-module-body rator-val)))))))
