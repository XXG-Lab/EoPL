...
(define (apply-subst search-var subst)
  (cases substitution subst
    ...
    (extended-subst (subst var ty)
                  (if (equal? search-var var)
                      ty
                      (apply-subst search-var subst)))))

(define (extend-subst subst var val)
  (extended-subst subst var (newref val)))
...
(define (apply-subst-to-type ty subst)
  (cases type ty
    ...
    (tvar-type (sn)
               (let ((tmp (apply-subst ty subst)))
                 (if tmp
                     (let ((next-type (apply-subst-to-type (deref tmp) subst)))
