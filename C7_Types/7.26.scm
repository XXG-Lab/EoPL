...
(define (list-type->type ty)
  (cases type ty
    (list-type (ty) ty)
    (else (error 'list-type->type ty))))

(define (refer-type? ty)
  (cases type ty
    (refer-type (ty) #t)
    (else #f)))

(define (refer-type->type ty)
  (cases type ty
    (refer-type (ty) ty)
    (else (error 'refer-type->type ty))))
...
(define (apply-one-subst ty0 tvar ty1)
  (cases type ty0
    ...
    (refer-type (ty)
                (refer-type (apply-one-subst ty tvar ty1)))
    (multi-type (types)
...
(define (apply-subst-to-type ty)
  (cases type ty
    ...
    (list-type (ty)
               (list-type (apply-subst-to-type ty)))
    (refer-type (ty)
                (refer-type (apply-subst-to-type ty)))
    (multi-type (types)
  ...
  (let ((ty1 (if (tvar-type? ty1) (apply-subst-to-type ty1) ty1))
    ...
    (cond ((equal? ty1 ty2))
          ...
          ((and (list-type? ty1) (list-type? ty2))
           (unifier (list-type->type ty1) (list-type->type ty2) exp))
          ((and (refer-type? ty1) (refer-type? ty2))
           (unifier (refer-type->type ty1) (refer-type->type ty2) exp))
  ...
  (cases expression exp
    ...
    (list-exp (exp exps)
              (let ((ty (type-of exp tenv))
                ...
                (map-of-two (lambda (type exp) (unifier ty type exp)) types exps)
                (list-type ty)))
    (cons-exp (exp1 exp2)
              (let ((ty1 (type-of exp1 tenv))
                    (ty2 (type-of exp2 tenv)))
                (unifier (list-type ty1) ty2 exp)
                ty2))
    (car-exp (exp)
             (let ((ty (type-of exp tenv)))
               (cases type ty
                 (list-type (ty) ty)
                 (else (error 'type-of "Expression should be a list: " exp)))))
    (cdr-exp (exp)
             (type-of exp tenv))
    (null?-exp (exp)
               (let ((ty (type-of exp tenv)))
    ...
    (newref-exp (exp)
                (refer-type (type-of exp tenv)))
    (deref-exp (exp)
               (cases type (type-of exp tenv)
                 (refer-type (ty) ty)
                 (else (error 'type-of "Expression should be a reference: " exp))))
    (setref-exp (var exp)
                (let ((ty1 (type-of var tenv))
                      (ty2 (type-of exp tenv)))
                  (unifier ty1 (refer-type ty2) exp)
                  (void-type)))
    (zero?-exp (exp)
...
(define prog-list "list(1, 2, 3)")
(equal?! prog-list '(listof int))

(define prog-car "car(list(1, 2, 3))")
(equal?! prog-car 'int)

(define prog-cdr "cdr(list(1, 2, 3))")
(equal?! prog-cdr '(listof int))

(define prog-null "null?(list(1, 2, 3))")
(equal?! prog-null 'bool)

(define prog-emptylist "emptylist")
(equal?! prog-emptylist '(listof (var 55)))

(define prog-cons "cdr(cons(proc(n:?) n, emptylist))")
(equal?! prog-cons '(listof ((var 56) -> (var 56))))

(define prog-newref "newref(proc(n:?) n)")
(equal?! prog-newref '(refto ((var 58) -> (var 58))))

(define prog-deref "deref(newref(proc(n:?) n))")
(equal?! prog-deref '((var 59) -> (var 59)))

(define prog-setref "setref(newref(proc(n:?) -(n, 1)), proc(n:?) -(1, n))")
(equal?! prog-setref 'void)
