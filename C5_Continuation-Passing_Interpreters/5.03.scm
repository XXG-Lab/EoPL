...
(define-datatype continuation continuation?
  (sub-1-cont (exp2 expression?)
              (env environment?)
              (saved-cont continuation?))
  (sub-2-cont (val1 expval?)
              (saved-cont continuation?))
...
(define (apply-cont cont val)
  (cases continuation cont
    (sub-1-cont (exp2 env saved-cont)
                (let ((val1 val))
                  (value-of/k exp2 env (sub-2-cont val1 saved-cont))))
    (sub-2-cont (val1 saved-cont)
                (let ((val2 val))
