...
(define-datatype continuation continuation?
  ...
  (send-1-cont (exp expression?)
               (env environment?)
               (saved-cont continuation?))
  (send-2-cont (val expval?)
               (saved-cont continuation?))
  ...
  (if (time-expired?)
      ...
      (begin
        ...
        (cases continuation cont
          ...
          (send-1-cont (exp env saved-cont)
                       (value-of/k exp env (send-2-cont val saved-cont)))
          (send-2-cont (th saved-cont)
                       (send-message (expval->num th) val)
                       (apply-cont saved-cont val))
...
(define-datatype thread thread?
  (a-thread (id integer?)
            ...
            (messages (list-of expval?))))

(define (get-thread-id t)
  (cases thread t
    (a-thread (id proc time-remaining messages)
              id)))

(define (new-thread proc)
  (a-thread (next-thread-id) proc the-max-time-slice (empty-queue)))

(define (new-current-thread proc)
  (a-thread current-thread-id proc the-time-remaining current-messages))

(define (restore-thread-time t)
  (cases thread t
    (a-thread (id proc time-remaining messages)
              (a-thread id proc the-max-time-slice messages))))
...
(define (send-message-rec q thread-id val)
  (if (empty-queue? q)
      '()
      (cases thread (car q)
        (a-thread (id proc time messages)
                  (if (equal? id thread-id)
                      (cons (a-thread id proc time (enqueue messages val)) (cdr q))
                      (cons (car q) (send-message-rec (cdr q) thread-id val)))))))

(define (send-message thread-id val)
  (if (equal? current-thread-id thread-id)
      (enqueue current-messages val)
      (set! the-ready-queue (send-message-rec the-ready-queue thread-id val))))
...
(define current-messages 'uninitialized)
...
(define (initialize-scheduler! ticks)
  ...
  (set! max-thread-id 0)
  (set! current-messages (empty-queue)))
...
(define (value-of/k exp env cont)
  (cases expression exp
    ...
    (yield-exp ()
               (place-on-ready-queue! (new-current-thread (lambda () (apply-cont cont (num-val 0)))))
               (run-next-thread))
    (kill-exp (exp)
              (value-of/k exp env (kill-cont cont)))
    (send-exp (th exp)
              (value-of/k th env (send-1-cont exp env cont)))
    (receive-exp ()
                 (if (empty-queue? current-messages)
                     (begin (place-on-ready-queue! (new-current-thread
                                                    (lambda()
                                                      (value-of/k exp env cont))))
                            (run-next-thread))
                     (let ((val (car current-messages)))
                       (set! current-messages (cdr current-messages))
                       (apply-cont cont val))))))
...
(define program-message "var dummy; { dummy =
letrec rec() = print(receive)
in let a = spawn(rec)
   in begin
          yield;
          send(a, 24)
